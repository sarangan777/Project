...
if ($servisor_id <= 0) {
    redirectWithMessage('services.php', 'Please select a service provider to book.', 'error');
}

// Get servisor details
$servisor = getServisorDetails($conn, $servisor_id);
if (!$servisor || !$servisor['is_approved'] || !$servisor['is_active']) {
    redirectWithMessage('services.php', 'Service provider not found or not available.', 'error');
}
